...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False, epsilon=1.0, alpha=0.5,tol=0.05):
...
waypoint)
	...
	self.maxQind=max(self.Q[state],key=self.Q[state].get)
	#print "Q[state] is",self.Q[state]
	#print "maxQind = ", self.maxQind
        maxQ = self.Q[state][self.maxQind]
	...
	if self.learning==True:
		...
		if rand>(self.epsilon*100):
			...
			maxacts=[]
			for acts in self.valid_actions:
				#print "maxQ value for state is",self.get_maxQ(state)
				#print "Q value for",acts,"is",self.Q[state][acts]
				if self.get_maxQ(state)==self.Q[state][acts]:
					maxacts.append(acts)
...
def run():
    ...
    sim = Simulator(env, update_delay=0.01, optimized=True, log_metrics=True, display=False)
